...
### 本地技能（.claude/skills / .Codex/skills）
...
| 子节 | 内容要求 | 测试关注点 |
|------|---------|-----------|
| 8.N.1 需求说明 | 功能目标+触发条件+前置依赖+涉及角色+映射产品目标(G1~GN) | 触发条件是否完备、前置依赖是否可构造 |
| 8.N.2 产品规则 | 原子化规则列举，每条独立子弹点，格式 `[PR-N.X] 描述` | 每条规则可测试，与验收标准一一对应 |
| 8.N.3 边界条件与异常场景 | 四分区：输入边界/并发竞态/降级容灾/存量兼容；缺失项标 `[待确认:T-XX]` | 系统性补全，避免遗漏异常场景 |
| 8.N.4 字段定义 | 三层字段表：页面展示→API→数据库/索引，均含「是否本期新增」列 | 字段覆盖完整，新增字段无遗漏 |
| 8.N.5 页面交互 | 操作入口(精确到菜单)+交互流程+二次确认规则+提示/错误文案(精确到字) | 文案可校验，入口可定位 |
| 8.N.6 状态流转 | 有生命周期：Mermaid stateDiagram-v2 + 转移条件表；无则声明"本功能无生命周期状态" | 状态路径可遍历 |
| 8.N.7 验收标准 | AC编号 + Given-When-Then 格式，一条一个断言，余力标（余力）| Given 前置条件不许省略 |
| 8.N.8 补充约束 | 安全口径/审计/i18n/部署开关等非通用规则，格式 `[约束编号] 类别 \| 描述 \| 影响范围` | 约束有编号可追溯 |
...
6. 字段定义三层覆盖
7. 边界条件四分区至少各一条
8. **移出功能零残留**：模块文件中不做（移出本期）的功能内容必须完全移除，仅在 `00_文档头与通用章节.md` §4a 中记录。禁止在模块文件内以注释、删除线、标注等形式保留移出内容
...
|------|------|------|
| 产品目标 | G + 数字 | G1, G6 |
| 用户故事 | US + 数字 | US-2, US-10 |
| 功能模块 | F + 数字 | F1, F11 |
| 产品规则 | PR + 模块号.序号 | PR-4.1, PR-8.3 |
| 验收标准 | AC + 模块号.序号 | AC-1.1, AC-16.10 |
| 待确认事项 | T + 数字 | T-01, T-18 |
